...
function generateFactSheetPdf({ productUuid, productName, values, completeness, channel, labels = {} }) {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      // Sanitise strings for PDF literal string syntax (escape parens and backslash)
      const pdfStr  = s => s.replace(/\\/g, '\\\\').replace(/\(/g, '\\(').replace(/\)/g, '\\)');

      // Right-align: approximate character width at 7pt Helvetica ≈ 3.9px/char
      const rightX  = 50 + W - (right.length * 3.9);

      doc.save();
      // Draw separator line
      doc.addContent(`0.82 0.82 0.82 RG 0.5 w ${50} ${fY + 6} m ${50 + W} ${fY + 6} l S`);
      // Left text
      doc.addContent(`BT /Helvetica 7 Tf 0.61 0.64 0.69 rg ${50} ${fY} Td (${pdfStr(left)}) Tj ET`);
      // Right text
      doc.addContent(`BT /Helvetica 7 Tf 0.61 0.64 0.69 rg ${rightX.toFixed(1)} ${fY} Td (${pdfStr(right)}) Tj ET`);
      doc.restore();
    }
