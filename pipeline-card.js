...
function generateFactSheetPdf({ productUuid, productName, values, completeness, channel, labels = {} }) {
  return new Promise((resolve, reject) => {
    ...
    // doc.text() always moves the cursor, which causes pdfkit to auto-add
    // blank pages when called after switchToPage(). The only safe approach
    // is to write the footer text directly into the page's content stream
    // using raw PDF operators via doc.addContent() — this bypasses the text
    // flow engine and the cursor entirely.
    //
    // IMPORTANT: raw PDF uses bottom-up coordinates (origin = bottom-left).
    // pdfkit's high-level methods use top-down. For raw addContent() we must
    // convert: pdfY = page.height - topDownY
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      // Top-down Y of footer text: 30px from the bottom of the page
      const topDownY = pageH - 30;
      // Convert to PDF bottom-up Y
      const pdfY     = 30;  // = pageH - topDownY = pageH - (pageH - 30) = 30

      const left  = 'Generated by Akeneo PIM  \xB7  Esko Asset Decompactor';
      const right = `Page ${i + 1} / ${totalPages}`;

      // Sanitise strings for PDF literal string syntax
      const pdfStr = s => s.replace(/\\/g, '\\\\').replace(/\(/g, '\\(').replace(/\)/g, '\\)');

      // Approximate right-align: Helvetica 7pt ≈ 3.9 units/char
      const rightX = 50 + W - (right.length * 3.9);

      doc.save();
      // Separator line: from (50, pdfY+8) to (50+W, pdfY+8) in PDF coords
      doc.addContent(`0.82 0.82 0.82 RG 0.5 w ${50} ${pdfY + 8} m ${50 + W} ${pdfY + 8} l S`);
      // Left-aligned footer text
      doc.addContent(`BT /Helvetica 7 Tf 0.61 0.64 0.69 rg ${50} ${pdfY} Td (${pdfStr(left)}) Tj ET`);
      // Right-aligned footer text
      doc.addContent(`BT /Helvetica 7 Tf 0.61 0.64 0.69 rg ${rightX.toFixed(1)} ${pdfY} Td (${pdfStr(right)}) Tj ET`);
      doc.restore();
    }
