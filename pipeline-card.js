...
function generateFactSheetPdf({ productUuid, productName, values, completeness, channel, labels = {} }) {
  return new Promise((resolve, reject) => {
    ...
    // ── Footer — drawn via pageAdded event, never via switchToPage() ────
    // Guard flag prevents re-entrancy: doc.text() inside drawFooter could
    // itself trigger another pageAdded if the text somehow overflows,
    // which would cause blank pages and an infinite loop.
    let pageNumber    = 0;
    let inFooterDraw  = false;

    const drawFooter = () => {
      if (inFooterDraw) return;  // prevent re-entrant calls
      inFooterDraw = true;
      pageNumber++;
      const fY = doc.page.height - 36;
      doc.save();
      // Move cursor explicitly above footer area so text never overflows
      doc.y = fY - 10;
      ...
      doc.fillColor(LGREY).font('Helvetica').fontSize(7)
         .text(`Page ${pageNumber}`, 50 + W / 2, fY, { width: W / 2, align: 'right', lineBreak: false });
      doc.restore();
      inFooterDraw = false;
    };

    // pageAdded fires for every page EXCEPT the first — draw it manually here
    drawFooter();
