...
function generateFactSheetPdf({ productUuid, productName, values, completeness, channel, attrMeta = {}, groupMeta = {} }) {
  return new Promise((resolve, reject) => {
    ...
    //
    // pdfkit's CTM = [1 0 0 -1 0 pageH] flips Y. We cancel it by applying
    // the same matrix again (self-inverse), giving normal PDF bottom-up space.
    // Footer text at "30pt from bottom" = y=30 in that unflipped space.
    ...
    for (let i = 0; i < totalPages; i++) {
      ...
      const textY  = 30;   // 30pt from page bottom in unflipped PDF space
      ...
      // Separator line via pdfkit vector API (CTM-aware, renders correctly)
      doc.save();
      doc.moveTo(50, pageH - 38).lineTo(50 + W, pageH - 38)
         .strokeColor('#D1D5DB').lineWidth(0.5).stroke();
      doc.restore();

      // Cancel pdfkit's Y-flip CTM with its inverse (same matrix = self-inverse),
      // draw text in normal upright orientation, then Q restores the CTM.
      doc.addContent([
        'q',
        `1 0 0 -1 0 ${pageH} cm`,
        `BT /Helvetica 7 Tf 0.61 0.64 0.69 rg ${50} ${textY} Td (${pdfStr(left)}) Tj ET`,
        `BT /Helvetica 7 Tf 0.61 0.64 0.69 rg ${rightX.toFixed(1)} ${textY} Td (${pdfStr(right)}) Tj ET`,
        'Q',
      ].join('\n'));
    }
